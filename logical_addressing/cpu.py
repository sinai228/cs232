...
class CPU:
        ...
        while True:
            ...
            if self._debug:
                ...
                print("CPU {}: executing code at [{}]: {}".format(self._num, self._registers['pc'],
                                                          self._mmu.addressCheck(self._registers['pc'])))

            # Execute the next instruction.
            self.parse_instruction(self._mmu.addressCheck(self._registers['pc']))
    ...
    def _get_value_at(self, addr):
        ...
        or hex.'''
        addr = eval(addr[1:])
        return self._mmu.addressCheck(addr)
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.addressTranslation(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.addressTranslation(eval(dst), srcval)
    ...
    def handle_add(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                temp_srcvall = self._mmu.addressCheck(self._registers[dst[1:]]) + srcval
                self._mmu.addressTranslation(self._registers[dst[1:]], temp_srcvall)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            temp_srcvall = self._mmu.addressCheck(eval(dst)) + srcval
            self._mmu.addressTranslation(eval(dst), temp_srcvall)
    ...
    def handle_sub(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                temp_srcvall_minus = self._mmu.addressCheck(self._registers[dst[1:]]) - srcval
                self._mmu.addressTranslation(self._registers[dst[1:]], temp_srcvall_minus)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            temp_srcvall_minus = self._mmu.addressCheck(eval(dst)) - srcval
            self._mmu.addressTranslation(eval(dst), temp_srcvall_minus)
