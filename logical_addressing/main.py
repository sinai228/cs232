...
class Monitor:
    ...
    def _handle_data_label(self, addr, line, pcb): 
        if len(line.split()) != 2:
            raise ValueError("Illegal format: __main: must be followed by memory address.")
        data_length = int(line.split()[1])
        high = addr + data_length
        pcb.set_high_mem(high)
        if self._debug:
            print("The important information", high, "has been read and stored in the high memory location of PCB.")
