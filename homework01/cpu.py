...
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, num=0):
        ...
        # TODO: store Boolean for defining batchmode
        # code for CS 232 homework01
    ...
    #code for CS 232 homework 01
    def set_batch(self, batch_val):
        # TODO: sets the batchmode with value of boolean, taken in as an argument?
        assert isinstance(batch_val, bool)
        self._batchmode = batch_val
    ...
    def run(self):
        #code for CS232 homework01
        #calling the program to run in batchmode if True
        if self._batchmode:
            self._run_batch()
    ...
    #code written for CS232 homework01
    def _run_batch(self):
        #starting location in the array of addr, as program counter
        self._ram[0] = self._registers['pc']
        mem_addr = self._ram [self._ram[0]]

        #call run_program for each program on the list
        while self._ram [self._ram [0]] != 0:
            #set program counter 
            self._registers['pc'] = mem_addr
            #call function _run_program
            self._run_program()
            #increment the array
            self._ram[0] += 1
            #setting the memory address to the next address in the array
            mem_addr =  self._ram [self._ram[0]]
